...
pub fn load_decoded(
    ...
    let (key, source) = resolve_source(src, base_dir, mode)?;
    if let Some(cached) = cache().lock().ok().and_then(|c| c.get(&key).cloned()) {
        return cached;
    }
    let result = fetch_and_decode(src, &source);
...
fn fetch_and_decode(src: &str, source: &ImageSource<'_>) -> CachedLoad {
    let (bytes, resource_dir) = fetch_bytes(source)?;
    let decoded = if is_svg(src, &bytes) {
        rasterize_svg(&bytes, resource_dir.as_deref())
    } else {
        decode_raster(&bytes)
    };
...
<svg xmlns="http://www.w3.org/2000/svg" version="1.1"
  ...
  <image href="side.png" x="0" y="0" width="8" height="8"/>
    ...
    fn cache_hit_serves_decode_without_rereading_the_file() {
        ...
        let first = load_decoded(path.to_str().unwrap(), None, ImageMode::LocalOnly)
            .expect("first load decodes");
        std::fs::write(&path, b"GARBAGE, not an image").unwrap();
        set_mtime(&path, mtime);
        let second = load_decoded(path.to_str().unwrap(), None, ImageMode::LocalOnly)
    ...
    fn failed_decode_is_cached_until_the_file_changes() {
        ...
        assert_eq!(
            load_decoded(src, None, ImageMode::LocalOnly).err(),
            Some(ImageUnavailable::Failed)
        );
        ...
        assert!(load_decoded(src, None, ImageMode::LocalOnly).is_ok());
    }
    ...
    fn cache_stays_bounded_under_many_distinct_images() {
        ...
        for i in 0..(IMAGE_CACHE_CAP + 8) {
            ...
            assert!(load_decoded(&uri, None, ImageMode::LocalOnly).is_ok());
